...
export const useCurrentWalletValue = (portfolio: PortfolioType[], values: ValueType[]) => {
  ...
  const currentWalletValueArr = portfolio.map(p => values
    .map(v => v.id === p.id && +(p.valueCount * +v.priceUsd)).reduce((acc: number, num: boolean | number) => +acc + +num, 0));
  const currentWalletValue = currentWalletValueArr.reduce((acc: number, num: boolean | number) => +acc + +num, 0).toFixed(2);
  const originalWalletValue = portfolio.map(p => p !== null && p.valueCount * p.price).reduce((acc: number, num: boolean | number) => +acc + +num, 0).toFixed(2);
